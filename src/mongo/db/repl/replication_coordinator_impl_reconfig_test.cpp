...
//    ASSERT_EQ(getReplCoord()->getTerm(), 1);
//
//    // Advance your optime and commit this op in this config.
//    auto opTime = OpTime(Timestamp(2, 1), 1);
//    replCoordSetMyLastAppliedAndDurableOpTime(opTime);
//    ASSERT_OK(getReplCoord()->setLastAppliedOptime_forTest(configVersion, 2, opTime));
//
//    // Do a force reconfig that should succeed.
//    configVersion = 3;  // will not be used by a force reconfig.
//    ReplSetReconfigArgs args;
//    args.force = true;
//    args.newConfigObj =
...
//    ASSERT_OK(status);
//
//    // Do a safe reconfig that should succeed even though the last committed op from the previous
//    // config is not committed in the current config.
//    configVersion = getReplCoord()->getConfig().getConfigVersion() + 1;
//    args.force = false;
//    args.newConfigObj =
//        configWithMembers(configVersion,
//                          BSON_ARRAY(member(1, "n1:1") << member(2, "n2:1") << member(3, "n3:1")
