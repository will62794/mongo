...
//    //
//    // In this test there are three configs. We go from C1 -> C2 -> C3. We ensure that before
//    // moving from C2 to C3 any ops committed in C1 are committed in C2.
//    //
//    // If a node is currently a secondary, it will not process any non force reconfig commands,
//    // so it will not keep track of the last op committed in the previous config. So, if it
//    // becomes primary and tries to process a reconfig command to go from Ci -> Cj, it may
//    // incorrectly think that it is safe to make this transition, even if ops committed in
//    // configs prior to Ci were not yet committed in Ci. So, we must wait until this new primary
//    // commits an op in its own term, so we are sure it has all ops committed in previous
//    // configs.
//    //
//    //
//
...
//
//
//    // If you try to do a reconfig before you have committed an entry in our own term as primary,
//    // you should fail.
//
//    // Get elected primary.
//    simulateSuccessfulV1Election();
...
//    ASSERT_OK(status);
//
//    unittest::log() << "### Stepping down node";
//    getReplCoord()->stepDown(opCtx.get(), true, Milliseconds(0), Milliseconds(1000));
//
//    // If the node steps up again, it should start a new term.
//    unittest::log() << "### Simulating new election";
//    simulateSuccessfulV1Election();
//    unittest::log() << "### My new term: " << getReplCoord()->getTerm();
//
//    log() << "### Starting reconfig (3) in separate thread.";
////    auto reconfigOpCtx = makeOperationContext();
//    reconfigThread = stdx::thread(
//        [&] { status = getReplCoord()->processReplSetReconfig(opCtx.get(), args, &result); });
//
//    // Satisfy the quorum check.
